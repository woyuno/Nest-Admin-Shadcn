...
import { configDetailQueryKey, fetchConfigDetail } from '../api/configs'
...
export function DataTableRowActions({ row }: DataTableRowActionsProps) {
  ...
  const { setOpen, setCurrentRow } = useConfigs()
    ...
    return (
      <DropdownMenu modal={false}>
        ...
        <DropdownMenuContent align='end' className='w-[160px]'>
          <PermissionGuard permissions={['system:config:edit']}>
            <DropdownMenuItem
              onPointerEnter={() => {
                void queryClient.prefetchQuery({
                  queryKey: configDetailQueryKey(row.original.configId),
                  queryFn: () => fetchConfigDetail(row.original.configId),
                  staleTime: 60 * 1000,
                })
              }}
              onClick={() => {
